...
api_columns = ['API/DLL_1', 'API/DLL_2', 'API/DLL_3', 'API/DLL_4', 'API/DLL_5', 'API/DLL_6', 'API/DLL_7', 'API/DLL_8', 'API/DLL_9', 'API/DLL_10', 'API/DLL_11', 'API/DLL_12', 'API/DLL_13', 'API/DLL_14', 'API/DLL_15', 'API/DLL_16', 'API/DLL_17', 'API/DLL_18', 'API/DLL_19', 'API/DLL_20', 'API/DLL_21']
...
desired_dll_features = {
'API/DLL_21' : [
    "CRYPT32.dll",
    "KERNEL32.dll",
    "WS2_32.dll", #WIdows 소켓 2 API DLL
    "USER32.dll",
    "ADVAPI32.dll", #고급 windows API함수를 포함한 레지스트리 작업 DLL
    "SHELL32.dll"  #SHELL 작업 수행하는 함수 DLL
]
}
...
def extract_api_functions(file_path):
    try:
        ...
        api_functions = [imp.name.decode('utf-8') for entry in pe.DIRECTORY_ENTRY_IMPORT for imp in entry.imports]
        return api_functions
    except Exception as e:
        return []

def extract_dlls(file_path):
    try:
        pe = pefile.PE(file_path)
...
def pre_api(path):
    ...
    dlls = extract_dlls(path)
    label_values = [0] * 21  # 그룹 레이블별 카운터를 저장하기 위한 리스트 초기화
    i=0
    # api/dll_0 ~ api/dll_20
    for index, group_features in desired_features.items():
        label_value = 0
        for feature in group_features:
            if feature in apis:
                label_value += 1  # API 함수가 발견될 때마다 카운터 증가
        label_values[i] = label_value  # 카운터를 저장
        i+=1
    # api/dll_21
    for index, group_features in desired_dll_features.items():
        label_value = 0
        for feature in group_features:
            if feature in dlls:
                label_value += 1
        label_values[20] = label_value
    return label_values
